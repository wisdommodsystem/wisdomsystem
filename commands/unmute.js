...
module.exports = {
  ...
  execute: async (message, args) => {
    ...
    if (!message.guild.members.me.permissions.has(PermissionsBitField.Flags.MuteMembers)) {
      return message.reply('❌ I need the "Mute Members" permission to unmute users. Please ask an administrator to grant this permission.');
    }
    ...
    if (!target.moderatable) {
      ...
      return message.reply(`❌ I cannot unmute this user because:\n` +
    ...
    try {
      ...
      if (!target.voice.serverMute) {
        return message.reply('❌ This user is not server muted.');
      }
      ...
      if (error.code === 50013) {
        return message.reply('❌ I don\'t have permission to unmute users. Please make sure I have the "Mute Members" permission and my role is above the target user\'s role.');
      }
      return message.reply(`❌ Failed to unmute ${target.user.tag}. Error: ${error.message}`);
    }
  }
}; 